...
(function($) {
    ...
    function Plugin($container, options) {
        ...
        if (self.options.sharedId) {
            $track = $(document).find("#" + self.options.sharedId + "-track");
            $canvas = $track.find("canvas");
        ...
        function _initialize() {
            if(hasCanvas) {
                var noCanvasTag = !$canvas || $canvas.length === 0;
                if (noCanvasTag) {
                    $canvas = $("<canvas></canvas>");
                    $track.append($canvas);
                }

                // I think this needs to be set for all cases
        ...
        function _setImage() {
            ...
            $(colorPicker).triggerHandler("load");
        ...
        function _bindPickerClicks() {
            if (hasCanvas) {
                if (!touchEvents) {
                    $canvas.mousedown(function(event) {
                        mouseIsDown = true;

                        _getColorCanvas(event, $container);
                    ...
                    $canvas.bind("touchstart", function(event) {
                        ...
                        _getColorCanvas(event.originalEvent.touches[0], $container);
         ...
         * @private
         */
        function _getColorCanvas(event, $invokingContainer) {
            if(mouseIsDown) {
                ...
                self.setColor("rgb(" + colorData[0] + "," + colorData[1] + "," + colorData[2] + ")", $invokingContainer);
                ...
                if (self.options.change) {
                    self.options.change($container, self.colorHex, self.colorRGB);
                }
